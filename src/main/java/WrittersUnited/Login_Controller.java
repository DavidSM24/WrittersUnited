...
public class Login_Controller {
	...
	public void forgot() {
		...
		if (txt_username.getText().matches("")) { 
			...
			if(UserDAO.getByName(txt_username.getText()) != null) {
				User user=UserDAO.getByName(txt_username.getText());
				Alert alert = new Alert(AlertType.CONFIRMATION);
				alert.setHeaderText(null);
				alert.setTitle("Confirmación");
				...
				if (result.get() == ButtonType.OK){
					...
					try {
						MailSender.sendMail(user.getMail(), "Recuperación de contraseña para Writters United", "Su contraseña para iniciar sesión en Writters United es la siguiente: "+user.getPassword());
